...
BEGIN_EVENT_TABLE(ImageBufferApp, wxApp)
END_EVENT_TABLE()
...
BEGIN_EVENT_TABLE(MainFrame, wxFrame)
    EVT_MENU(ID_MENU_RENDER_START, MainFrame::OnRenderStart)
...
RenderCanvas::RenderCanvas(wxWindow* parent)
    : wxScrolledWindow(parent), m_image(NULL), scene(NULL), thread(NULL), timer(NULL), updateTimer(this, ID_RENDER_UPDATE)
{
...
void RenderCanvas::SetImage(wxImage& image)
{
    ...
    SetScrollbars(10, 10, (int)(m_image->GetWidth() / 10.0f), (int)(m_image->GetHeight() / 10.0f), 0, 0, true);
...
void RenderCanvas::OnNewPixel(wxCommandEvent& event)
{
    ...
    std::vector<RenderPixel*> *pixelsUpdate = (std::vector<RenderPixel*> *)event.GetClientData();

    for (std::vector<RenderPixel*>::iterator itr = pixelsUpdate->begin(); itr != pixelsUpdate->end(); itr++)
    {
...
void RenderCanvas::OnTimerUpdate(wxTimerEvent& event)
{
    ...
    wxString progressString = wxString::Format("Rendering...%d%%", (int)(completed * 100));
    ...
    long msecRemain = (long) (((double)interval / (completed*100)) * 100 * remaining);
