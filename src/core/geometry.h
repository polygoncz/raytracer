...
public:
	...
	const Normal& operator+() const { return *this; }
...
{
public:
	Ray() : mint(0.f), maxt(), time(0.f), depth(0) { }
	Ray(const Point& _o, const Vector& _d, float start = 0.f,
		float end = INFINITY, float _time = 0.f, int _depth = 0)
		: o(_o), d(_d), mint(start), maxt(end), time(_time), depth(_depth) { }
