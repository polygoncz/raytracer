...
bool Scene::Intersect(const Ray& ray, Intersection& inter) const
{
	...
	for (vector<Shape*>::const_iterator itr = objects.begin(); itr != objects.end(); itr++)
	{
		Shape* obj = (*itr);
		if (obj->CanIntersect())
		{
		...
		else
		{
			...
			for (vector<Shape*>::const_iterator ref_shape = refined->begin(); ref_shape != refined->end(); ref_shape++)
			{
